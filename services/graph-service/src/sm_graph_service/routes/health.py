...
`/readyz` probes the Kafka producer, the Kafka consumer, and Neo4j — if any is
unreachable the service cannot apply commands, so it should be pulled from
rotation (the consumer keeps retrying in the background regardless).
"""
...
def _checks(services: Services) -> list[DependencyCheck]:
    return [
        ...
        probe_check("neo4j", services.graph, required=True),
    ]

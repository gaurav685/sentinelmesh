...
`/readyz` requires PostgreSQL and the Kafka producer — nothing else. Every
content dependency (`detection-engine`, `graph-service`, `mitre-service`,
`ai-analyst`, `memory-service`) and object storage are soft: their absence
degrades a generated report to `partial` / `failed`, it does not take the
whole service down.
"""
...
def _checks(services: Services) -> list[DependencyCheck]:
    bucket = services.settings.s3_bucket_reports
    return [
        probe_check("postgres", services.db, required=True),
        probe_check("kafka_producer", services.producer, required=True),
        DependencyCheck(
            "object_storage", lambda: services.store.ping(bucket), required=False,
        ),
    ]

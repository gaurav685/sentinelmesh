...
`EVENT_PAYLOAD_REGISTRY` (Phase 2, Unit 1). As of Phase 2 the whole pipeline runs against Redpanda: `ingestion-gateway`
produces the five `telemetry.*` payloads onto `telemetry.raw` (Unit 3), and
`normalization-engine` consumes them and produces `event.canonical`
(`CanonicalEventPayload`) onto `events.canonical` (Unit 4). Both round-trips are
integration-tested. The six payloads are now **exercised by both a producer and
a consumer** — the STABLE bar — but remain **STABLE target** until Phase 2 exits
CI. `partition_key` for every producer is `sm_contracts.make_partition_key`

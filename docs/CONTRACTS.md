...
| `BenchmarkExperiment` | ml-training (write, plain asyncpg) / api-gateway (read) | Postgres | **STABLE** (IMPLEMENTED P15; MLflow not yet deployed) | 15 |
...
| Date | Change | Phase |
|---|---|---|
| 2026-09-14 | **Phase 17 Security & Hardening Contracts** (Phase 17): HTTP response hardening enforces strict `Content-Security-Policy: default-src 'none'; frame-ancestors 'none'` in `SecurityHeadersMiddleware`. Internal service-to-service JWT contract (`sm_common.security.jwt_internal`) validates mandatory non-empty `audience` and non-empty `signing_keys` on mint and decode. Bounded query safety enforced on decoy listings (`DecoyRepository.list_decoys`). | 17 (close) |
| 2026-09-12 | **Phase 15 closed** (Unit 4): Grafana provisioning, benchmark-result storage + BFF read path, and a second real observability bug found by running the finished feature. New `sm_contracts.api.benchmark.BenchmarkExperiment` (read shape of a real `benchmark_experiment` row — every field is a genuinely executed measurement, `executed` always `True`, enforced by the table's own `CHECK`, not convention). New Postgres `benchmark_experiment` table (migration `0014`; `sm_common.db.BenchmarkExperimentRow` — no `tenant_id`, R24's own security-boundary line) + `sm_ml_training.benchmark.persist.save_benchmark_run` (plain `asyncpg`, new `ml-training[persist]` extra — `ml-training` stays an offline CLI, not a server) + a new `--save-to-db PG_DSN` CLI flag. `api-gateway`'s `SqlBenchmarkRepository` reads it directly (mirroring `SqlSocRepository`/`ContentRepository`'s established precedent for a BFF reading a table its owner exposes no HTTP read API for): `GET /api/v1/soc/benchmarks` (list, newest first, optional `dataset_id` filter) + `GET .../benchmarks/{id}`, gated on `ops:read` (verified against real seeded RBAC — only `platform_operator` actually holds it). New frontend `/benchmarks` page (list + lookup, populated only from real runs). `deploy/grafana/provisioning` auto-provisions the Prometheus datasource + a 14-panel "Platform Overview" dashboard; real bug caught bringing it up for real — a second bind mount nested inside the first's target directory fails on Docker Desktop ("read-only file system" creating the nested mountpoint), fixed by moving the dashboard JSON under the one `provisioning` mount. **Second real bug found by running the finished dashboard against live data, not by reading code:** `Metrics.observe_http` — declared, scraped, and dashboarded since an earlier phase — had no caller anywhere in the platform; fixed with a new `sm_common.fastapi.MetricsMiddleware`, wired into all 14 services, with one documented, tested, narrow gap (an unhandled `Exception` records status `0`, not the real `500` the client receives; every intentional `SmError` response is unaffected). Real, executed end-to-end proof: a real CLI benchmark run → a real Postgres insert → a real authenticated BFF read returning the exact row; a real `curl` against Grafana's own API; a real Prometheus query showing real HTTP-metric samples. 101 JSON Schema files (+1). Phase 15 exit report + §23 review in `IMPLEMENTATION_STATE.md`; `REQUIREMENTS_TRACEABILITY` R23/R24 → IMPLEMENTED with explicit deviation notes (MLflow, the three unusable datasets, Loki/Tempo/Jaeger collectors, and the status-0 gap all named, not rounded up). | 15 (close) |

...
- **Verification method:** **REAL, evidence-backed, partially verified (Phase 16) — see `docs/architecture/kubernetes-deployment.md`'s "Verification status" and `docs/IMPLEMENTATION_STATE.md`'s Phase 16 exit report for the full evidence trail.** Namespaces, all 6 stateful StatefulSets, all 14 app Deployments, NetworkPolicy, PodSecurity admission enforcement, ConfigMap/Secret (both providers), and the deliberate absence of RBAC all verified against a real local `kind` cluster. The migration Job, ingress, frontend, and HPA/KEDA autoscaling are `NOT VERIFIED — REQUIRES FURTHER INVESTIGATION` (migration) or were out of scope for this pass (the rest) — no cluster rollout claim is made beyond what that section documents.

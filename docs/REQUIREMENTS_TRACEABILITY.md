...
- **Test:** `helm lint` + `helm template` (both `secretsProvider` modes) static validation; real `kind` cluster install (Phase 16).
- **Verification method:** **REAL, evidence-backed, verified (Phase 16 & Phase 17) — see `docs/architecture/kubernetes-deployment.md`, ADR-025, and `docs/IMPLEMENTATION_STATE.md`'s Phase 16/17 exit reports for the full evidence trail.** Phase 17 added 64 security tests, 4 E2E flow tests, static Dockerfile/Helm/migration assertions, CI `security` gate with pip-audit, Trivy container scanning, and pinned base image digests. Namespaces, all 6 stateful StatefulSets, all 14 app Deployments, NetworkPolicy, PodSecurity admission enforcement, ConfigMap/Secret (both providers), and the deliberate absence of RBAC all verified against a real local `kind` cluster. The migration Job, ingress, frontend, and HPA/KEDA autoscaling are `NOT VERIFIED — REQUIRES FURTHER INVESTIGATION` (migration) or were out of scope for this pass (the rest) — no cluster rollout claim is made beyond what that section documents.
- **Phase:** P1 provides local `docker-compose` + auth/RBAC/SSO foundation; P16 built the Kubernetes/Helm layer. **Status:** IMPLEMENTED / PARTIALLY REAL-VERIFIED. The **RBAC and SSO foundation is IMPLEMENTED / LOCALLY VERIFIED** (Phase 1, Units 3-4): permission catalogue and system roles seeded by migration `0002`, deny-by-default `require_permission`, server-side principal resolution, per-request privilege re-resolution, and the OIDC authorization-code + PKCE client — verified only against in-memory fakes, not a real IdP. Kubernetes/Helm (Phase 16): real chart, real namespace bootstrap, real `kind` install with real bugs found and fixed; the migration Job's real, reproducible connection failure in a short-lived pod (not resolved) means no claim is made that the app tier runs correctly against a real migrated schema.
